...
if (isset($_POST['update_address'])) {
  if (isset($_SESSION['uid'])){
    ...
    if(!mysqli_query($con, $sql)){
        ...
        echo '<script>window.location="update-my-address.php"</script>';
    }
    else{
...
if (isset($_POST["delete"])){
	...
	if(mysqli_query($con,$sql)){
    echo "<script>window.alert('Product is Removed from cart')</script>";
    echo '<script>window.location="#"</script>';
	}
}
...
if (isset($_POST["confirmOrder"])){
  if (isset($_SESSION["uid"])) {
    ...
    if(mysqli_query($con,$sql)){
      echo "<script>window.alert('Your payment process is successfully completed')</script>";
